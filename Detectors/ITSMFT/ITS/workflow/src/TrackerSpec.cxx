  ...
  if (continuous) {
    for (auto& rof : rofs) {
      ...
      if (nclUsed) {
        ...
        trackLabels.swap(mTracker->getTrackLabels(roFrame)); /// FIXME: assignment ctor is not optimal.
    ...
    allTrackLabels.swap(mTracker->getTrackLabels(0)); /// FIXME: assignment ctor is not optimal.
  }
