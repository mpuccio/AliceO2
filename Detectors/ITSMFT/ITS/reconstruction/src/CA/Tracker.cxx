...
template<bool IsGPU>
Tracker<IsGPU>::Tracker(const Event &event) : mEvent{event}
{
  // Nothing to do
}
...
template<bool IsGPU>
std::vector<std::vector<Road>> Tracker<IsGPU>::clustersToTracksTimeBenchmark(std::ofstream& timeBenchmarkOutputStream)
{
  const int verticesNum = mEvent.getPrimaryVerticesNum();
  std::vector<std::vector<Road>> roads;
  roads.reserve(verticesNum);
