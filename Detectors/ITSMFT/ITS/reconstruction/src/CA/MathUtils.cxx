...
{
  return std::atan2(-yCoordinate, -xCoordinate) + Base::Constants::kPI;
}
...
float MathUtils::computeCurvature(float x1, float y1, float x2, float y2, float x3, float y3)
{
...
float MathUtils::computeCurvatureCentreX(float x1, float y1, float x2, float y2, float x3, float y3)
{
  const float k1 = (y2 - y1) / (x2 - x1), k2 = (y3 - y2) / (x3 - x2);
  return 0.5f * (k1 * k2 * (y1 - y3) + k2 * (x1 + x2) - k1 * (x2 + x3)) / (k2 - k1);
}

float MathUtils::computeTanDipAngle(float x1, float y1, float x2, float y2, float z1, float z2)
{
