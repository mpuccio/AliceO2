    ...
    for (int iCell{0}; iCell < layerCellsNum; ++iCell) {
      ...
      if (nextLayerFirstCellIndex != constants::its::UnusedIndex &&
        ...
        for (int iNextLayerCell{nextLayerFirstCellIndex};
             ...
             mTimeFrame->getCells()[iLayer + 1][iNextLayerCell].getFirstTrackletIndex() ==
               nextLayerTrackletIndex;
             ++iNextLayerCell) {
...
void Tracker::findTracks()
{
  ...
  for (auto& road : mTimeFrame->getRoads()) {
    ...
    bool fitSuccess = fitTrack(temporaryTrack, constants::its::LayersNumber - 4, -1, -1);
    if (!fitSuccess)
      continue;
    CA_DEBUGGER(fitCounters[nClusters - 4]++);
    temporaryTrack.resetCovariance();
    fitSuccess = fitTrack(temporaryTrack, 0, constants::its::LayersNumber, 1);
    if (!fitSuccess)
      continue;
    CA_DEBUGGER(backpropagatedCounters[nClusters - 4]++);
    temporaryTrack.getParamOut() = temporaryTrack;
    temporaryTrack.resetCovariance();
    fitSuccess = fitTrack(temporaryTrack, constants::its::LayersNumber - 1, -1, -1);
    if (!fitSuccess)
      continue;
...
bool Tracker::fitTrack(TrackITSExt& track, int start, int end, int step)
{
  track.setChi2(0);
