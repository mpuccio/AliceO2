...
bool Tracker::fitTrack(TrackITSExt& track, int start, int end, int step, const float chi2cut)
{
  ...
  for (int iLayer{start}; iLayer != end; iLayer += step) {
    ...
    if (mMatLayerCylSet) {
      if ((iLayer + step) != end) {
        const auto cl_0 = mTimeFrame->getClusters()[iLayer][track.getClusterIndex(iLayer)];
        const auto cl_1 = mTimeFrame->getClusters()[iLayer + step][track.getClusterIndex(iLayer + step)];
