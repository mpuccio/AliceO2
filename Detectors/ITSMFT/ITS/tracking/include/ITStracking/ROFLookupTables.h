...
// GPU-friendly view. The fixed-capacity table remains templated, but its view
// is runtime-sized so it can cross into the detector-neutral core.
...
// GPU-friendly view of the table below. Its layer extent is carried in the
// view so the core need not know which fixed-capacity table built it.
template <int32_t NLayers, typename TableEntry, typename TableIndex>
