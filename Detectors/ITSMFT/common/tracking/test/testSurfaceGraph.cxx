...
SurfaceGraph buildGraph(const std::vector<SurfaceDescriptor>& surfaces, std::vector<SurfaceId> order)
{
  ...
  BOOST_REQUIRE(graph.addTransition(SurfaceTransition{SurfaceId{3}, SurfaceId{2}, {}, 0}).isValid());
  BOOST_REQUIRE(graph.addTransition(SurfaceTransition{SurfaceId{1}, SurfaceId{0}, {}, 0}).isValid());
...
BOOST_AUTO_TEST_CASE(ViewCarriesOneOwnerCatalogOrderAndTopology)
{
  ...
  BOOST_CHECK_EQUAL(view.nTransitions, 2u);
  BOOST_CHECK_EQUAL(view.nCells, 0u);
  BOOST_CHECK(view.getTransition(TransitionId{0}).from == SurfaceId{3});
  BOOST_CHECK(view.getTransition(TransitionId{0}).to == SurfaceId{2});
}
...
BOOST_AUTO_TEST_CASE(ViewIsDeviceSafePOD)
{
  ...
  BOOST_CHECK(copy.transitions == view.transitions);
...
BOOST_AUTO_TEST_CASE(NonContiguousGlobalIdsUseOneGraphLookup)
{
  ...
  BOOST_CHECK(view.getTransition(TransitionId{0}).from == SurfaceId{7});
