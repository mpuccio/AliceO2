...
//
// Gate 4 B1 Slice C3a: focused tests for ITSSurfaceSpec/MFTSurfaceSpec.
//
// Two independent things are proved here, deliberately not conflated:
//
// 1. Each authored nominalReferenceCoordinate literal reproduces the exact
//    float32 bit pattern of the source JSON token it was mechanically
//    transcribed from (Gate 4 B1 Slice 1 detached validator's --format
//    json output, Gate 4 acceptance-cleanup C1's lossless encoding). The
//    source tokens are embedded here as string literals -- copied
//    verbatim from the recorded provenance, not re-derived -- and decoded
//    with std::strtof (not std::from_chars: unavailable for float at this
//    build's deployment target, see the C1 commit); the comparison is a
//    bitwise memcpy'd uint32_t ==, zero tolerance, matching
//    validation/test/testLosslessFloatRoundTrip.cxx's technique. This is
//    NOT a geometry-comparison/validator mode: no geometry file is loaded,
//    no GeometryTGeo is touched, nothing is aggregated -- only two static
//    strings are compared against two static literals already compiled
