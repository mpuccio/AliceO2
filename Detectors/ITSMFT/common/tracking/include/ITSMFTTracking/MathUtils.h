...
#ifndef O2_ITSMFT_TRACKING_MATHUTILS_H_
#define O2_ITSMFT_TRACKING_MATHUTILS_H_
...
namespace o2::itsmft::tracking::math_utils
{
...
GPUhdi() float hypot(float x, float y)
{
...
GPUhdi() float computeCurvatureCentreX(float x1, float y1, float x2, float y2, float x3, float y3)
{
  ...
  if (o2::gpu::CAMath::Abs(dx21) < o2::itsmft::tracking::constants::Tolerance ||
      o2::gpu::CAMath::Abs(dx32) < o2::itsmft::tracking::constants::Tolerance) { // add small offset
    x2 += 1e-4;
    dx21 = x2 - x1;
    dx32 = x3 - x2;
  }
  const float k1 = (y2 - y1) / dx21, k2 = (y3 - y2) / dx32;
  if (o2::gpu::CAMath::Abs(k2 - k1) < o2::itsmft::tracking::constants::Tolerance) {
...
GPUhdi() float computeTanDipAngle(float x1, float y1, float x2, float y2, float z1, float z2)
{
  ...
  if (o2::gpu::CAMath::Abs(d) < o2::itsmft::tracking::constants::Tolerance) {
