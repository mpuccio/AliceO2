...
- `StateFamily` describes the representation carried by a
...
| `stateFamilyFromNLayers<NLayers>()`, `detIdFromNLayers<NLayers>()`, and `if constexpr` family branches in traits | These are hidden 7/10 selectors. Some are adapter identity; the state family itself is representation metadata. | 3 at adapter edges; 4 in core | Resolve representation from the actual `SurfaceDescriptor` and the state's `StateFamily`. Do not use the mapping as a new central dispatch type. |
...
- no `StateFamily` value selects scheduling or topology;
