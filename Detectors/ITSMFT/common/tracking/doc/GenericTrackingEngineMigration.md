...
Post-M7 cleanup implementation: [sequential Luna execution plan](design/0010-post-m7-cleanup-implementation-plan.md) — L3 complete; L4 is the only next authorized slice. [L3 SurfaceGraph validation](validation/l3-surface-graph.md) records the one-owner/view result and the passive `TimeFrame`/active `Tracker::initialize` boundary.
