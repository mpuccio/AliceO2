...
| Gate 2: common CA traversal | In progress | D007, device-compatible cylinder/disk policy parameter and state-family traits, host outer-family dispatch, the first host CPU policy operation (`cellsAreCompatible`), and an explicit-order layout builder with exact legacy graph parity and a disconnected 7+10 construction test are integrated. The production TimeFrame has transactional, geometry-epoch-aware ownership of a canonical surface catalog and per-iteration layouts, with current-only bounds-checked access. Shared geometry aggregation and thin ITS/MFT providers derive active-area bounds from aligned sensor transforms; the MFT common-CA workflow now constructs and retains this layout observationally, with real-geometry validation and unchanged documented replay metrics/track-content hash. Normalized multi-source loading now consumes a narrow catalog-only POD view, while the TimeFrame compatibility boundary derives both its canonical catalog and layer mapping from the same current owned layout-set snapshot; zero-iteration catalog loading, stale/missing ownership, detector qualification, allocation-free mapping preflight, and transactional failure are covered by focused tests. A reproducible replay also records the pre-migration common-CA physics, repeatability, and resource baseline alongside Gate 0 without claiming parity with the legacy tracker. The legacy duplicate formula and the detector branch in that hot loop are removed; topology-driven TrackerTraits/CATracker orchestration, ITS production migration, and ITS/MFT algorithm parity remain. |
