...
| Gate 2: common CA traversal | Complete | D007, typed cylinder/disk policy state and one-shot host dispatch, explicit-order layout construction, geometry-epoch-aware TimeFrame catalog/layout ownership, and sparse-topology scheduling for `findCellsNeighbours` are integrated. Bounded normalized decoding rejects malformed, truncated, missing-dictionary, and trailing-pattern inputs transactionally. `wipe()` clears normalized event state while preserving layout configuration, and normalized-to-legacy backfill has an allocation-failure-safe staged commit with corrected memory-resource lifetime. All 16 common tracking suites pass on the integration branch. The canonical 20-event common-CA replay remains identical at the metrics-JSON level (91 tracks; content hash `826dc653cd936a472929c600c97c140b`), with no material paired wall-time or RSS regression. Independent correctness/physics and architecture blocker reviews accepted the gate; subsequent CA-operation migration and ITS production activation belong to Gate 3. |
| Gate 3: opt-in production migration | Accepted with explicit limits | D009 coexistence is integrated: legacy defaults remain unchanged and `o2-its-ca-tracker-workflow` is an isolated opt-in common-CA workflow. Fresh-build/test evidence, deterministic ITS common-CA replays (203 tracks; state hash `ee7f7c794d60f2362fd2564258b7887e`), expanded ITS attachment checks (1262/1262), and the MFT fence (70 tracks; hash `24737e73b7146bf3bd35a90a2517c527`; 450/450 attachments) are accepted. Scope is Sync-only, timestamped static-diamond constrained, tracker-only/no vertex products, with no GPU, Async/Cosmics/UPC, real vertexer, or native ITS refit activation. The legacy ITS reference is explicitly non-equivalent; no matched-constraint A/B claim is made. |
| Gate 4: combined disconnected tracking | C2/C3/C4 integrated; acceptance evidence complete | D010's wiring plan is implemented and committed: the combined 17-surface catalog (`kITSMFTCombinedStaticSurfaceCatalog`, `StaticDetectorCatalogs.h`) is now the authoritative combined-catalog source, with per-detector static tables remaining adapter/application data for the single-detector path only. `DetectorTraversalBinding` is wired into every `TrackerTraits.cxx` hot loop that indexes `LegacyTrackerScratch<NLayers>` by `TransitionId`/`CellTopologyId`/road-start ID (global traversal binding). `Tracker<NLayers>::clustersToTracks()` returns a typed tracking outcome instead of wiping the shared TimeFrame itself, and the `findRoads()` gap in that typed-outcome coverage is closed. The MFT refit identity check is sourced from the active binding rather than a detector-local assumption, with an end-to-end test covering the correction (source-identity refit fix). `CombinedTimeFrameCoordinator` (host-only, unwired to DPL) builds the one combined 17-surface topology exactly once and derives both detectors' bindings from that single build -- proven byte-identical rather than independently re-derived -- and classifies combined-TF failures per detector's `DropTFUponFailure` into `RecoverableDropped`/`Structural` outcomes (combined host coordinator). The coordinator reproduces a genuine nonzero accepted ITS `CommonTrack` alongside MFT's, with tracklet/cell parity against independent single-detector-catalog standalone runs proving no cross-detector topology was traversed (exact ITS output), and MFT's existing float-based `TrackingFrameInfo`-projected publication path carries through unchanged into the combined coordinator's output staging (approved MFT float-projected output). `o2-itsmft-combined-ca-tracker-workflow` (isolated opt-in combined workflow) wraps the coordinator as a DPL device, links neither `O2::ITSCAWorkflow` nor `O2::MFTWorkflow`, runs five independent local preflight checks, and now shares one contract-tested `O2::ITSMFTCAWriter` library with both legacy single-detector writers instead of duplicating their writer specs. A clean, pinned `daily-20260717-0700-local1` skill environment reran the exact eight-test serial selector, passing 8/8 including `testCombinedTimeFrameCoordinator` and `testGate4B31OwnershipContract`. The installed `MathCore.pcm`'s stale transient BUILD-tree string is benign and does not affect test correctness. The only infrastructure issue reproduced is the known parallel ACLiC compiled-macro race on unrelated macro tests, which does not occur in the serial review selector used here. No ROOT/O2 package file was modified. Acceptance evidence is now complete; the remaining limits are architectural deferrals only (`LegacyTrackerScratch`, the two-detector source/binding bridge, and `TransitionPolicyTag`/policy cleanup), not an unresolved validation failure. |
