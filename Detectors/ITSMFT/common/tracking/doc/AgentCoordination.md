...
| Gate 2: common CA traversal | In progress | D007, device-compatible cylinder/disk policy parameter and state-family traits, host outer-family dispatch, the first host CPU policy operation (`cellsAreCompatible`), and an explicit-order layout builder with exact legacy graph parity and a disconnected 7+10 construction test are integrated. The production TimeFrame now has transactional, geometry-epoch-aware ownership of a canonical surface catalog and per-iteration layouts, with current-only bounds-checked access. A reproducible replay of the production MFT common-CA workflow records its pre-migration physics, repeatability, and resource baseline alongside Gate 0 without claiming parity with the legacy tracker. The legacy duplicate formula and the detector branch in that hot loop are removed; concrete geometry catalog providers, production layout configuration, topology-driven TrackerTraits/CATracker orchestration, and ITS/MFT algorithm parity remain. |
