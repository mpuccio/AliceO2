...
| Gate 4: combined disconnected tracking | C2 wiring plan accepted; implementation not started | D010 records the accepted wiring audit: the combined 17-surface catalog, `DetectorTraversalBinding` (global-ID-to-scratch-slot translation), and `MultiSourceTimeFrameLoader::loadITSAndMFT()`/`resetITSAndMFTEvent()` (combined load and combined reset) are already committed and unit-tested but have zero production/coordinator call sites. The plan traces every `TransitionId`/`CellTopologyId`/road-start site in `TrackerTraits.cxx` that indexes `LegacyTrackerScratch<NLayers>`, specifies the minimal `adoptDetectorTraversalBinding()` API, moves whole-TF wipe/drop ownership from `Tracker<NLayers>` to a future coordinator via a typed tracking outcome, and confirms `CommonTrack` publication is already safe (append-only, monotonic sidecars) for serial ITS-then-MFT. No `TimeFrame<17>`, second topology/catalog, provider/epoch system, or mixed tracks are introduced. Remaining work is Slice 1 (`TrackerTraits`/`Tracker` wiring), Slice 2 (typed failure outcome), and a test-only combined coordinator -- no DPL/production coordinator is authorized by this gate status. |
