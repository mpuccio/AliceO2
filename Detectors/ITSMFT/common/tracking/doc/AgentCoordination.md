...
| Gate 4: combined disconnected tracking | C2/C3/C4 integrated; M6f/M6g cleanup complete | The combined 17-surface catalog, `SurfacePlanBinding`, `SurfaceTrackingScratch`, and explicit ITS/MFT schedule remain the authoritative application composition. The former host-only coordinator and participant-set composition have been folded into the combined DPL task; event-owned publication clocks, validity, sidecar reset/invalidation, ROF timing setup, and source-qualified publication are workflow-owned. The generic tracking library retains only `TrackingEngine`, `TrackingParticipant`, `TimeFrame`, the plan-driven participant adapters, and the detector-neutral workspace/binding/seed model. The existing C2/C3/C4 replay and writer evidence remains the behavior baseline; M6g adds direct-composition ownership tests and a production dependency guard. The remaining limits are architectural deferrals only (Tracker/TrackerTraits `NLayers` de-templating and the separately scoped `TransitionPolicyTag`/policy cleanup), not an unresolved M6g validation failure. |
