...
| Gate 2: common CA traversal | In progress | D007, device-compatible cylinder/disk policy parameter and state-family traits, host outer-family dispatch, the first host CPU policy operation (`cellsAreCompatible`), and an explicit-order layout builder with exact legacy graph parity and a disconnected 7+10 construction test are integrated. The production TimeFrame has transactional, geometry-epoch-aware ownership of a canonical surface catalog and per-iteration layouts, with current-only bounds-checked access. Shared geometry aggregation and thin ITS/MFT providers derive active-area bounds from aligned sensor transforms; normalized multi-source loading consumes a narrow catalog-only POD view, while the TimeFrame compatibility boundary derives both its canonical catalog and layer mapping from the same current owned layout-set snapshot. `findCellsNeighbours` is the first production operation driven by the sparse topology: a deterministic graph-derived schedule and one-shot policy grouping/binding replace numeric-layer traversal and the detector branch, with exact legacy-index parity preflight and fail-closed MFT workflow behavior. All 15 common tracking suites pass on the combined integration branch, and a fresh 20-event replay is byte-for-byte identical at the metrics-JSON level to the canonical common-CA baseline (91 tracks; content hash `826dc653cd936a472929c600c97c140b`); paired whole-workflow characterization finds no material wall-time or RSS regression. Gate 2 remains open for the full-stack acceptance review; ITS production activation, subsequent CA-operation migration, and ITS/MFT production parity belong to the reviewed Gate 3 plan. |
