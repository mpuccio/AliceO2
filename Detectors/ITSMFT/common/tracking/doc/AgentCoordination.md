...
| Gate 3: opt-in production migration | In progress | D009 fixes the coexistence boundary: independent legacy and common jobs consume identical pinned inputs and conditions, the common path fails without automatic fallback, and default switching or legacy retirement is not a Gate 3 deliverable. The first bounded common-only slices are failure-contract correction and `attachHit` policy dispatch; ITS parameter/workflow onboarding and semantic A/B validation follow without changing legacy algorithm behavior. |
